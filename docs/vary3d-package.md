...
- **Do not invent a split** — if the source has a `part` enum, keep it; DOCUMENT `## Print` lists Print N× per token. Do not add `variants.json` presets that only switch `part`.
...
| info.json, covers, DOCUMENT, README, ORIGIN | [references/package.md](../skills/vary3d-package/references/package.md) |
| Print in DOCUMENT `## Print` | [references/print.md](../skills/vary3d-package/references/print.md) |
