...
- **Split for print:** user saying “split” / “easier” → Confirm with a recommendation, not auto-split. If it stays one piece, **tell the user**. After they pick split: 2–4 kinds of **one article** → one `part` enum, default All. Tokens share envelope + `wall`. No `show_<part>`. Extra files for a second complete product. `params.scad` only if opening A cannot export B’s piece. `--single-body` per token, not on `all`.
- **Few knobs:** millimetres in the request are defaults, not sliders. Visible cap: simple ≤ 6, complex ≤ 8. Mate holder: object sizes only. Split of one article: no per-token envelopes. Three layers: visible knobs, Hidden literals, derived locals inside the module. `extract-params.py` warnings block Done.
- **`params.scad`:** only for a **kit** (complementary pieces on different files that must share wall / footprint / clearance). Kit test: opening A cannot export B’s piece. Not for a single file, a `part` split, extra Models that are each a full product, or libraries.
