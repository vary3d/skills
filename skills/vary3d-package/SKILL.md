...
description: >-
  ...
  variants.json, cover images, DOCUMENT.md, README.md, LICENSE, and ORIGIN.md. Does not invent
...
- **New file copy** (`info.json` name/description/tags, variant titles, Customizer groups and slider `//` lines you add, `DOCUMENT.md`, `README.md`) defaults to **English**. Switch only when the user **explicitly** asks (e.g. “listing in Japanese”, or any other language they name). Chatting in another language is **not** that request.
...
packages/<slug>/
  ...
  DOCUMENT.md         # Long-form; Import → Docs
  README.md           # GitHub landing page (derived)
...
- Listing copy, covers, DOCUMENT, README, ORIGIN → [references/package.md](references/package.md)
- Print notes in DOCUMENT `## Print` → [references/print.md](references/print.md)
...
10. Do not write server-assigned fields: `id`, `userId`, object-storage paths, `status`, `visibility`, `__vary`. Do not write `print` (`validate-info.py` rejects it; use DOCUMENT `## Print`). **Never invent `parentModelId`** — omit it unless you already have a real Vary3D id.
...
# DOCUMENT.md (Import Docs) + derived GitHub README.md
