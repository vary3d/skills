...
5. `info.json`: `description` leads with object + mate (e.g. round flange, M5 holes); more sentences optional. Category `practical_gadgets`; tags from object / mate / feature (`flange`, `m5`, `bolt` — about 3, max 5). `print` three lines. `validate-info.py`.
...
8. `cover.py` then `cover-variants.py`. `validate-variants.py`. Open covers.
9. Leave `models/` (or the original `flange.scad`) untouched.
